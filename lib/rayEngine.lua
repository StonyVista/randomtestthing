...
function rayEngine.drawDebugInformation(x, y, width, transparency, ...)
	local lines = {...}
	buffer.drawRectangle(x, y, width, #lines, 0x000000, 0x000000, " ", transparency); x = x + 1
	for line = 1, #lines do buffer.drawText(x, y, 0xEEEEEE, lines[line]); y = y + 1 end
end
...
local function drawFieldOfViewAngle(x, y, distance, color)
	...
	buffer.drawSemiPixelLine(x, y, xFirst, yFirst, color)
	buffer.drawSemiPixelLine(x, y, xSecond, ySecond, color)
end
...
function rayEngine.drawMap(x, y, width, height, transparency)
	...
	buffer.drawRectangle(x, y, width, yHalf, 0x000000, 0x000000, " ", transparency)
...
function rayEngine.intro()
	...
	local function draw(transparency)
		...
		buffer.drawImage(x, y, logo)
		buffer.drawRectangle(1, 1, buffer.getWidth(), buffer.getHeight(), 0x000000, 0x000000, " ", transparency)
		buffer.drawChanges()
		os.sleep(0)
	end
...
function rayEngine.compass(x, y)
	...
	buffer.drawImage(x, y, rayEngine.compassImage)
	...
	y = y * 2
	buffer.drawSemiPixelLine(x, y, northPoint.x, northPoint.y * 2, 0xFF5555)
	buffer.drawSemiPixelLine(x, y, southPoint.x, southPoint.y * 2, 0xFFFFFF)
...
function rayEngine.watch(x, y)
	...
	buffer.drawImage(x, y, rayEngine.watchImage)
	x, y = x + 15, y + 12
	...
	y = y * 2
	buffer.drawSemiPixelLine(x, y, xMinute, yMinute * 2, 0xEEEEEE)
	buffer.drawSemiPixelLine(x, y, xHour, yHour * 2, 0xEEEEEE)
end	
...
function rayEngine.chat(transparency)
	...
	buffer.drawRectangle(x, y, rayEngine.chatPanelWidth, rayEngine.chatPanelHeight, 0x000000, 0xFFFFFF, " ", transparency or 0.5)
	...
	for message = #rayEngine.chatHistory, 1, -1 do
		for line = #rayEngine.chatHistory[message].text, 1, -1 do
			buffer.drawText(x, yMessage, rayEngine.chatHistory[message].color or 0xFFFFFF, rayEngine.chatHistory[message].text[line])
			yMessage = yMessage - 1
...
function rayEngine.drawWeapon()
	if rayEngine.currentWeapon.needToFire then buffer.drawImage(rayEngine.currentWeapon.xFire, rayEngine.currentWeapon.yFire, rayEngine.currentWeapon.fireTexture); rayEngine.currentWeapon.needToFire = false end
	buffer.drawImage(rayEngine.currentWeapon.xWeapon, rayEngine.currentWeapon.yWeapon, rayEngine.currentWeapon.weaponTexture)
	buffer.drawImage(rayEngine.currentWeapon.xCrosshair, rayEngine.currentWeapon.yCrosshair, rayEngine.currentWeapon.crosshairTexture)
end
...
function rayEngine.drawStats()
	...
	buffer.drawRectangle(x, y, width, height, 0x000000, 0xFFFFFF, " ", 0.5)
...
function rayEngine.drawWorld()
	...
	--Небо
	buffer.drawRectangle(1, 1, buffer.getWidth(), rayEngine.horizonPosition, rayEngine.world.colors.sky.current, 0x0, " ")
	--Сцена
	...
	for angle = startAngle, endAngle, rayEngine.raycastStep do
		...
		if distanceToTile then
			...
			if rayEngine.properties.useSimpleRenderer then
				...
				buffer.drawRectangle(math.floor(startX), math.floor(startY), 1, math.floor(height), tileColor, 0x000000, " ")
			else
				...
				buffer.drawSemiPixelRectangle(math.floor(startX), math.floor(startY), 1, height, tileColor)
			end
			...
			-- 	buffer.drawImage(math.floor(startX), math.floor(startY), column)
			-- end
		end
		startX = startX + 1
	end
